...
// Moved out of Général — client references power CRM-style enrichment
// (matched against briefs the same way Pipedrive/HubSpot data is), so it
// belongs alongside the CRM connections, not the commercial profile form.
export default function ClientReferencesSection({
  ...
  return (
    <div className="mt-10">
      <FadeIn delay={0.1}>
        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Références clients</p>
      ...
      <ClientReferencesTable version={version} />
    </div>
  );
}
