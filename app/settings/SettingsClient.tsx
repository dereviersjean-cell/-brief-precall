...
export default function SettingsClient({
  ...
  useEffect(() => {
    ...
    if (crm === "hubspot_connected") {
      ...
      setRecallToast({ type: "success", message: "HubSpot connecté avec succès." });
      const t = setTimeout(() => setRecallToast(null), 5000);
      return () => clearTimeout(t);
    }
    if (crm === "error") {
      setRecallToast({ type: "error", message: "La connexion au CRM a échoué, réessayez." });
      const t = setTimeout(() => setRecallToast(null), 5000);
      return () => clearTimeout(t);
    }
  }, [searchParams]);
          ...
          <div className="space-y-5">
            <div>
              ...
              <p className="mt-1.5 flex items-center gap-1.5 text-xs text-slate-400">
                <svg className="w-3.5 h-3.5 text-indigo-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><circle cx="12" cy="12" r="10"/><path strokeLinecap="round" strokeLinejoin="round" d="M12 16v-4m0-4h.01"/></svg>
