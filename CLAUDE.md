...
- **« Performance » est une section à 3 onglets** (`app/components/PerformanceTabs.tsx`, réintroduit le 25/07/2026, même pattern que `TeamTabs`/`SettingsTabs`) : Vue d'ensemble (`/dashboard`), Historique (`/contacts`), Entraînement (`/training`) — Entraînement a rejoint Performance à cette date plutôt que de rester une entrée sidebar séparée. La sidebar ne pointe que vers `/dashboard` (`performanceActive` couvre les 3 routes). La Vue d'ensemble reste un digest (KPIs de la semaine + `RecentCallsList`), complété par deux cartes « Historique important » (top contacts récents via `getContactsOverview`, lien « voir tout » vers l'onglet Historique) et « Objections importantes » (top objections par fréquence + taux de succès via `getObjectionStatsForOrganization`, lien vers `/settings/objections` — Objections reste dans Paramètres, pas dans Performance). Même pattern côté `ManagerOverview.tsx`.
- **Sélecteur « vue équipe / commercial spécifique »** (`app/dashboard/CommercialSelector.tsx`, manager only) : `/dashboard?commercial=<id>` bascule l'onglet Vue d'ensemble sur la performance individuelle d'un commercial précis, en réutilisant `CommercialOverview` tel quel (`viewerRole="manager"`) — mêmes données, juste sourcées pour l'id sélectionné plutôt que le manager. Autorisation gratuite : la liste vient de `getCommercialsForManager` (déjà scopée aux commerciaux liés à ce manager) ; un id absent de cette liste retombe silencieusement sur la vue équipe, jamais une erreur qui confirmerait l'existence d'un id d'un autre manager/org. En mode manager, `CommercialOverview` masque `ConnectionsStatus` et les actions (Nouveau brief, Cette semaine, Exporter) — ces boutons n'agiraient de toute façon que sur le compte du manager connecté, pas celui du commercial consulté, donc trompeurs à laisser affichés.
