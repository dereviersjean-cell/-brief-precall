...
- `lib/transcript-import.ts` parse `.vtt`, `.srt`, `.json` (transcript brut Recall **et** notre propre `TranscriptJson`) et texte brut « Nom : phrase ». Repli sur le texte brut si un JSON est de forme inconnue.
- **Aucune durée n'est inventée** : sans horodatage réel dans la source, `transcriptJson` reste null et les métriques d'interaction ne sont pas calculées (l'UI le dit). Des timings estimés au nombre de mots produiraient des chiffres de patience et de monologue plausibles mais fabriqués.
