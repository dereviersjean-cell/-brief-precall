...
Reste à aligner un jour : `.env.local` porte encore l'ancienne clé et l'ancien `RESEND_FROM_EMAIL`, et la politique de confidentialité affiche `rgpd@infobrief.com`, domaine sans rapport avec Brief (le gel de l'examen Google est levé depuis l'approbation : plus rien n'interdit d'y toucher).

### Vérification Google — APPROUVÉE (fin août 2026)

**Google a approuvé la vérification** (confirmé par Jean ; l'approbation a suivi la réponse du 31/08, et le commentaire de `lib/auth.ts` la situe la veille du 01/09/2026). Ce qui tombe avec elle : l'écran « application non vérifiée » à la première connexion, le plafond de 100 utilisateurs, et les consignes de gel — scopes, branding, URL, `rgpd@infobrief.com` peuvent de nouveau être touchés. Ce qui NE tombe pas : les refresh tokens déjà morts, qui ne se réparent que par le bouton « Se reconnecter avec Google » (voir plus bas — au 03/09/2026 les 4 comptes ne l'ont pas tous fait). **Tout ce qui suit est la trace du dossier, à ne pas relire comme une attente en cours.**
...
- ~~Ce qui subsiste jusqu'à la validation : l'écran « application non vérifiée » à la première connexion, et le plafond de **100 utilisateurs sur la durée de vie du projet, non réinitialisable**~~ — **levé par l'approbation de fin août 2026.** Conservé pour mémoire : c'était l'argument pour faire aboutir la vérification avant d'ouvrir les inscriptions.
...
1. ~~Vérification Google~~ — **APPROUVÉE fin août 2026** (section dédiée ci-dessus). L'écran « application non vérifiée » et le plafond de 100 utilisateurs sont levés, le gel scopes/branding/URL n'a plus cours. **« Back to testing » reste à ne jamais cliquer**, pour une raison indépendante de la vérification : ce serait rouvrir l'expiration des refresh tokens à 7 jours.
   **Ce qui reste, et ne dépend pas du code : les 4 comptes ne sont pas tous reconnectés** (état au 03/09/2026). Chacun doit passer par « Se reconnecter avec Google » dans Paramètres → Connexions — une déconnexion/reconnexion ordinaire rend un access token frais mais **aucun** refresh token (commentaire de `lib/auth.ts`). Tant que ce n'est pas fini, **ne pas retirer le `prompt=consent`** : ni celui du bouton de reconnexion, ni celui de `app/api/recall/google-oauth/start/route.ts` — c'est le seul chemin de réparation d'un jeton mort.
...
**Ce qui vient d'être réglé (31/08/2026)** : le domaine émetteur des emails (section dédiée ci-dessus) et le *chemin* de reconnexion Google — le bouton « Se reconnecter avec Google », qui s'affiche tout seul aux comptes concernés. **La reconnexion elle-même n'est pas terminée : au 03/09/2026 les 4 comptes ne l'ont pas tous faite.**

**Ce qui ne bloque plus** :
- ~~Google OAuth en mode Testing~~ — **réglé le 20/08/2026**. Projet passé *In production*, l'expiration des refresh tokens à 7 jours ne s'applique plus. Vérification **approuvée fin août 2026** — voir la section dédiée.
...
### Ajout manuel d'un rendez-vous (03/09/2026)

**Le bouton « Ajouter un RDV » de la barre du brief n'avait aucun `onClick`** : une coquille décorative, même défaut que les boutons PDF/Partage avant le 21/08.

- **Décision de périmètre (tranchée avec Jean avant d'écrire le code)** : un RDV manuel sert **uniquement à préparer/consulter un brief**. Il n'écrit rien dans l'agenda Google/Microsoft réel et Recall n'en a jamais connaissance — pas de lien visio, pas de bot d'enregistrement automatique. Cohérent avec le reste de l'app, où l'enregistrement suit toujours le vrai agenda connecté ; créer un vrai événement calendrier aurait ajouté la gestion des deux providers, un lien visio, et modifié l'agenda réel de l'utilisateur pour un gain hors sujet (le besoin exprimé est de préparer un brief pour un RDV que l'agenda ne voit pas, pas de le faire enregistrer).
- **Table `manual_meetings`** (migration **012**, exécutée en prod le 03/09/2026) : `title`, `company_name`, `contact_email` (optionnel), `meeting_time`, cadrée par `user_id`. Contrairement à un événement de calendrier réel, `company_name` est **saisi explicitement** par l'utilisateur, jamais deviné depuis un domaine email (cf. le panneau Contacts vide du 22/08) — il n'y a pas toujours de contact renseigné.
- `GET /api/calendar/events` **fusionne** désormais les RDV manuels des 7 prochains jours (même horizon que `getUpcomingMeetings`) avec les vrais événements Google/Microsoft, triés ensemble par heure de début. Les deux sources échouent indépendamment : un calendrier réel en erreur (token expiré) n'empêche pas d'afficher les RDV manuels, et une table manuelle absente (migration pas encore passée) n'empêche pas d'afficher le calendrier réel — même pattern `.catch()` de repli que le reste de l'app (cf. bug #14).
- `POST /api/calendar/manual-events` (création) et `DELETE /api/calendar/manual-events/[id]` (suppression, cadrée sur `user_id` — cf. bug #28) sont les deux seules routes d'écriture ; pas de route de modification, la suppression + recréation suffit pour ce volume d'usage.
- Carte badgée « Ajouté manuellement » avec un bouton de suppression optimiste dans `BriefToolClient.tsx` — les cartes des vrais événements de calendrier n'ont pas ce bouton (rien à supprimer côté Brief, l'événement vit dans Google/Microsoft).

### Qualité du brief généré — ancrage web et honnêteté (03/09/2026)

Signalé par Jean sur un cas réel (Bewtr/BE WTR) : le brief généré était « extrêmement vague » — du remplissage générique SaaS B2B sans aucun fait vérifiable sur l'entreprise.

- **Cause racine, pas spécifique à ce prospect** : Pappers renvoie `401 "plus assez de crédits"` depuis un temps indéterminé, avalé en silence par `enrichWithPappers` (`if (!res.ok) return null`, sans log). **Tous** les briefs se génèrent donc sans données légales officielles. Fix : `reportWarning` sur tout échec HTTP Pappers (`lib/pappers.ts`) — ne corrige pas le manque de crédits (décision business, toujours à trancher par Jean) mais ça ne reste plus invisible.
- **`web_search_20260209`** (filtrage dynamique) remplace la variante `_20250305` (`lib/brief-generator.ts`) — le modèle configuré (`claude-sonnet-4-6`) la supporte, et c'est le levier le plus direct contre un brief générique.
- **Consigne d'ancrage forcée** (system prompt par défaut ET contrainte serveur dans `buildUserPrompt`, même logique que le contrat JSON forcé) : ancrer overview/accroche dans un fait vérifiable trouvé sur le web, dire honnêtement « peu d'info disponible » plutôt qu'inventer un profil générique.
- **Régression trouvée en testant ce fix en conditions réelles** : le modèle trouvait bien BE WTR via la recherche web mais **doutait de la correspondance** avec « Bewtr » — « il est possible que ce soit une entité différente, en mode furtif » — parce que son secteur (filtration d'eau physique) ne collait pas à l'attente implicite « SaaS B2B » du contexte commercial. La consigne d'honnêteté du point précédent était appliquée trop largement : elle couvrait aussi le cas d'un résultat trouvé mais surprenant. Fix : faire confiance à un résultat qui correspond clairement au nom donné (orthographe/espacement/casse près), même si le secteur surprend — ne jamais présupposer le secteur du prospect à partir du profil du commercial. Réserver « peu d'information disponible » au cas où la recherche ne remonte VRAIMENT rien.
- **`admin_config.main_config` mis à jour manuellement en prod** (deux fois, pour les deux fix ci-dessus) : c'était une copie conforme de l'ancien défaut du code, pas une personnalisation (cf. bug #24) — sans cette mise à jour manuelle, les correctifs de code restaient sans effet sur les briefs réels d'Oliverlist.
- **Bug critique trouvé au passage, indépendant de tout ce qui précède** : aucune des 3 routes qui appellent `generateBrief` (`/api/generate-brief`, `/api/admin/test`, `/api/admin/test-brief`) n'avait de `maxDuration`. Mesuré en conditions réelles : ~54s pour un brief avec recherche web sur une entreprise déjà documentée (Doctolib) — la recherche web forcée ci-dessus a rendu ça bien plus lent qu'avant. Sans `maxDuration`, Vercel coupe la fonction à son délai par défaut, largement sous 54s : risque de timeout silencieux en prod. `maxDuration = 120` ajouté aux 3 routes.
- **Animation de génération** (`GeneratingProgress`, `app/brief/[id]/BriefClient.tsx`) recalée sur cette durée réelle (~54s) au lieu d'un minutage fixe (2s/4s/6s puis bloqué à 90%) : la barre progresse en continu sur une courbe asymptotique qui ne se fige jamais, et au-delà de 20s un message dit explicitement que ça peut prendre jusqu'à une minute. **S'affiche aussi en régénération** (`isRegenerating`), pas seulement à la première génération — avant, régénérer un brief existant ne montrait qu'un petit spinner dans le bouton avec l'ancien contenu grisé en fond ; le contenu existant est maintenant remplacé par la même animation plutôt que grisé, puisqu'il est de toute façon sur le point d'être remplacé.

### Fiche contact enrichie — Apollo.io (03/09/2026)

Suite directe du point précédent : Jean a demandé pourquoi le panneau « Contacts » restait vide. Réponse : **il l'était pour tout le monde, pas spécifique à Bewtr** — `app/brief/[id]/page.tsx` codait en dur `contacts: []`, et même le nom d'invité connu de Google Calendar n'était jamais transmis jusque-là.

- **Demande initiale** : poste, carrière, âge, statut décisionnaire, une mini présentation utile pour le commercial. **L'âge n'est fourni par aucun fournisseur B2B** (Apollo compris) — non suivi par ce type de service, et sensiblement RGPD à collecter sans finalité claire. Volontairement absent, dit explicitement à Jean plutôt que laissé de côté en silence.
- **Choix du fournisseur, avec un vrai détour** : Proxycurl (option de départ, roadmap point 12) a fermé le 4 juillet 2026 suite à un procès LinkedIn. Bright Data évalué ensuite — le plus solide juridiquement (victoires face à Meta et X, scraping de données publiques jugé légal), mais ses champs Poste/Expérience/Formation sont tombés à ~2% de remplissage depuis novembre 2025 (LinkedIn a durci ses contre-mesures) : inutilisable pour le besoin exact de Jean. **Apollo.io retenu** : pas un scraper LinkedIn en direct (agrège plusieurs sources), champ `seniority` structuré directement exploitable, gratuit jusqu'à 10 lookups/mois (`x-api-key`, `POST /api/v1/people/match?email=...`).
- **`lib/apollo.ts`** : `enrichContact(email)` dégrade proprement sans `APOLLO_API_KEY` (même pattern que `lib/pappers.ts`), `reportWarning` sur échec HTTP. **`seniorityLabel()` est un mapping déterministe en code** (c_suite/owner/founder/partner → décisionnaire probable direction ; vp/director → décisionnaire probable ou fort pouvoir d'influence ; manager/senior → probablement impliqué ; entry/intern → probablement pas décisionnaire), **jamais deviné par l'IA** — un fait structuré ne doit pas être ré-interprété par un LLM qui pourrait halluciner un niveau différent d'un appel à l'autre. `formatContactSummary()` construit le résumé de carrière (poste actuel + ancienneté + poste précédent) à partir des seuls faits qu'Apollo renvoie.
- **`lib/brief-generator.ts`** : bloc « CONTACT QUI SERA EN FACE DU COMMERCIAL » injecté dans le prompt pour personnaliser accroche/arguments au rôle de la personne (un CTO et un VP Sales n'ont pas les mêmes préoccupations) — **jamais demandé dans le JSON en retour** : les champs factuels (poste, séniorité, LinkedIn) sont attachés à la réponse côté serveur après coup, pour ne jamais dépendre de la fidélité d'une recopie par le modèle (même principe que l'ancrage par numéros de ligne des verbatims d'objection).
- **`app/api/generate-brief/route.ts`** : `enrichContact()` en parallèle de Pappers/actualités, uniquement si `contactEmail` est connu. La fiche contact est fusionnée dans la réponse **et** dans ce qui est sauvegardé via `saveBrief` — une relecture d'un brief en cache ne re-consomme donc pas de crédit Apollo (10/mois sur le plan gratuit, à ne pas gaspiller sur des rechargements de page).
- `lib/types.ts` (`Brief.contact`), `lib/brief-content.ts` (`adaptCachedContent`) et `BriefClient.tsx` (`adaptApiBrief` + panneau Contacts) transportent le contact de bout en bout — plus besoin de toucher `meeting.contacts`, resté à `[]` partout et maintenant mort.
- **Reste à faire (Jean)** : créer un compte Apollo.io (gratuit, pas de CB requise pour démarrer) et poser `APOLLO_API_KEY` sur `.env.local` + Vercel. Sans la clé, le panneau Contacts retombe proprement sur le seul email connu (aucune casse, juste pas d'enrichissement).

### Où en est le code

- **12 migrations** dans `migrations/`. **Toutes appliquées en prod**, y compris la **012 (`manual_meetings`) et la 011 (`rate_limit_events`), passées le 03/09/2026** — la limitation de débit partagée ne retombe donc plus en silence sur le limiteur en mémoire. **Vérifiée par requête, pas sur parole** (une table créée mais absente du cache PostgREST donnerait exactement le même repli silencieux qu'une table manquante) : `GET /rest/v1/rate_limit_events` répond `200 []`, le comptage `count=exact` renvoie `content-range: */0`, un `POST` renvoie `201` avec `id` et `created_at` servis par défaut, le `DELETE` de contrôle `204` — les trois chemins qu'emprunte réellement `checkSharedRateLimit` (compter, insérer, purger). (006 à 009 depuis fin juillet, **010 le 22/08/2026** — vérifiée de la même façon : PostgREST renvoie `briefs.meeting_title` à `null` au lieu d'une erreur).
...
1. ~~Google OAuth — sortir du mode Testing~~ — **FAIT le 20/08/2026**, et **vérification APPROUVÉE fin août 2026**. Projet *In production*, plus d'écran « application non vérifiée » ni de plafond de 100 utilisateurs : rien du côté Google ne retient l'ouverture des inscriptions. Reste à finir la reconnexion des comptes existants (voir « Ce qui bloque et ne dépend PAS du code »).
2. ~~**Exécuter la migration 011** (`rate_limit_events`) sur Supabase prod~~ — **FAIT le 03/09/2026** (SQL Editor Supabase), vérifié par requête et non sur parole : comptage, insertion et suppression répondent. La limitation de débit partagée est enfin opposable — elle ne voit plus une seule instance Vercel.
...
12. ~~Proxycurl LinkedIn — enrichissement contact~~ — **FAIT le 03/09/2026, mais avec Apollo.io, pas Proxycurl** (fermé le 4 juillet 2026 suite au procès LinkedIn — plus une option). Bright Data évalué en alternative : légalement plus solide, mais ses champs Poste/Expérience sont tombés à ~2% de remplissage depuis nov. 2025 (LinkedIn a durci ses contre-mesures). Apollo.io retenu à la place — pas un scraper LinkedIn en direct, gratuit jusqu'à 10 lookups/mois. Voir `lib/apollo.ts` et la section dédiée ci-dessous. **L'âge n'est fourni par aucun fournisseur B2B** — jamais suivi par ce type de service, volontairement absent de la fiche contact.
