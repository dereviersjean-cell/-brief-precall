...
- **« Performance » (`/dashboard`) est une page digest unique, sans onglets** : les KPIs de la semaine + `RecentCallsList` restent, complétés par deux cartes « Historique important » (top contacts récents via `getContactsOverview`, lien vers `/contacts` qui reste une page à part entière) et « Objections importantes » (top objections par fréquence + taux de succès via `getObjectionStatsForOrganization`, lien vers `/settings/objections`). Même pattern côté `ManagerOverview.tsx`. `app/components/PerformanceTabs.tsx` a été supprimé (l'ancien design à onglets Dashboard/Objections/Historique est obsolète).
