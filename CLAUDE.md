...
- **Des exemples rendus, pas seulement décrits** (`TourDemos.tsx`) : 6 maquettes réalistes — un brief, une analyse de call, et un panneau par onglet de Performance (Scores, Analytics, Objections, Playbook). Un compte neuf n'a aucune donnée, et pointer des écrans vides ne montre pas ce que le produit produit.
  - Ce sont des **maquettes**, pas les vrais composants alimentés en faux : injecter des données fictives dans des pages qui lisent la base côté serveur serait invasif, et une donnée fictive qui fuit en production est bien pire qu'une visite imparfaite. Chaque panneau porte une mention « Exemple » — non négociable.
  - Contenu délibérément spécifique (chiffres, noms, phrases réelles) : un exemple générique du type « Objection 1 » n'apprend rien.
  - Une étape « exemple » n'a pas de cible et s'affiche centrée, avec le même en-tête et le même pied que les bulles ancrées — sinon on aurait l'impression de changer d'outil en cours de visite.
...
- **Le parcours raconte la logique du produit, il ne décrit pas des écrans** : les trois sections sont présentées comme trois moments qui s'enchaînent (préparer → débriefer → progresser), une étape est consacrée au fait que Brief pousse ses résultats dans l'email et le CRM plutôt que d'attendre qu'on vienne, et la dernière referme la boucle. Décrire les écrans un par un n'explique pas pourquoi ils vont ensemble.
- **Chaque bulle dit OÙ on est et CE QU'on regarde** : un badge de phase (Repères / Préparer / Débriefer / Progresser) et une nature (`section` / `contenu` / `commande`). Sans cette distinction toutes les bulles se ressemblaient, et on ne savait pas si on regardait une zone ou un bouton.
- **Voile léger (0.38) + anneau franc**, pas l'inverse : c'est l'anneau qui désigne, pas l'obscurité. Un voile épais met un élément en avant tout en empêchant de le situer dans la page.
- **Flèche orientée vers la cible** (carré pivoté héritant du fond de la bulle) : sans elle on lit une explication sans savoir de quoi elle parle.
- **Progression segmentée** plutôt qu'un « 3 sur 9 » nu, qui ne dit pas s'il reste beaucoup.
- **Ancrer sur le conteneur visible, pas sur une div interne** : ancrée sur la rangée flex intérieure, la surbrillance coupait la carte en deux et le cadrage paraissait cassé. La cible est aussi amenée dans l'écran (`scrollIntoView`) avant d'être mesurée, et la bulle se place à droite d'une cible étroite, sous une cible large — jamais par-dessus la zone mise en avant.
